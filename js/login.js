...
class ShieldLoginPortal {
  ...
  completeBiometricScan() {
    try {
      window.ShieldLoginMode = 'biometric';
      if (this.scanInterval) {
        clearInterval(this.scanInterval);
        this.scanInterval = null;
      }
  ...
  logout() {
    if (window.ShieldEvents) {
      window.ShieldEvents.emit('logoutSuccess', {
        user: window.ShieldState.userRole || 'Citizen',
        role: window.ShieldState.userRole === 'admin' ? 'Command Admin' : (window.ShieldState.userRole === 'agent' ? 'Security Agent' : 'Citizen Operator')
      });
    }
