        ...
        if (this.decryptPin === expectedPin) {
          ...
          const interval = setInterval(() => {
            ...
            if (progress >= 100) {
              ...
              // Audit list log creation
              const selectPurpose = document.getElementById('decrypt-purpose-select');
              const purposeVal = selectPurpose ? selectPurpose.value : 'threat';
              const purposeMap = {
                threat: 'Active threat tracking',
                audit: 'Forensic Audit request',
                consent: 'Citizen consent verification'
              };
              const purposeText = purposeMap[purposeVal] || 'Active threat tracking';
              const roleTextMap = {
              ...
              const roleLabelMap = {
                officer: 'Clearance Level 1',
                agent: 'Clearance Level 2',
                admin: 'Clearance Level 3'
              };

              window.ShieldEvents.emit('blockchainOverrideCreated', {
                user: roleTextMap[role] || 'Officer Operator',
                role: role === 'admin' ? 'Command Admin' : (role === 'agent' ? 'Security Agent' : 'Officer Operator'),
                purpose: purposeText,
                target: this.activeCamId,
                clearance: roleLabelMap[role] || 'Clearance Level 1'
              });
